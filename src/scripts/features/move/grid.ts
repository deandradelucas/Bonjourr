...
export function gridMove(move: SimpleMove, id: WidgetName, direction: Direction): void {
    const grid = moveWidgetInGrid(move, id, direction)
...
export function canMove(move: SimpleMove, id: WidgetName, direction: Direction): boolean {
    try {
        return isValidGrid(
            moveWidgetInGrid(move, id, direction),
        )
    } catch (_) {
        return false
    }
...
/** Step 0 */
function moveWidgetInGrid(move: SimpleMove, id: WidgetName, direction: Direction): Grid {
    ...
    addGridEdges(clone, direction, cloneWidget)
    optimisticSwap(clone, id, cloneWidget, direction)
...
function fixGridCollisions(move: SimpleMove, sizes: WidgetSizes, id: WidgetName): void {
    for (let _ = 0; _ < 100; _++) {
        ...
        for (const collidingId of MOVE_WIDGETS) {
            ...
            const notRectangle = findOffendingRow(move.grid, collidingId)
...
/** Step 3.1 */
function pushWidgetDown(move: SimpleMove, sizes: WidgetSizes, pushedId: WidgetName, lockedId: WidgetName): void {
...
function findOffendingRow(grid: Grid, id: string): number | undefined {
    /**
